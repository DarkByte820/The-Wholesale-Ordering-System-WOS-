...
class Payment {
    ...
    public function createPayment($order_id, $provider, $amount, $payment_method) {
        ...
        $stmt = $this->db->prepare("INSERT INTO payment (OrderID, Provider, Amount, Status, PaymentMethod, TransactionReference) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    public function getPayment($payment_id) {
        $stmt = $this->db->prepare("SELECT * FROM payment WHERE PaymentID = ?");
    ...
    public function getPaymentByOrder($order_id) {
        $stmt = $this->db->prepare("SELECT * FROM payment WHERE OrderID = ?");
    ...
    public function updatePaymentStatus($payment_id, $status) {
        $stmt = $this->db->prepare("UPDATE payment SET Status = ?, VerificationDate = NOW() WHERE PaymentID = ?");
    ...
    public function verifyPayment($transaction_ref) {
        $status = 'Successful';
        $stmt = $this->db->prepare("UPDATE payment SET Status = ?, VerificationDate = NOW() WHERE TransactionReference = ?");
    ...
    private function getPaymentByRef($transaction_ref) {
        $stmt = $this->db->prepare("SELECT * FROM payment WHERE TransactionReference = ?");
