...
class ReportController {
    ...
    public static function getInventoryReport() {
        ...
        $stmt = $conn->prepare("SELECT 
            ...
            SUM(CASE WHEN stock_quantity <= ReorderLevel THEN 1 ELSE 0 END) as low_stock_items,
    ...
    public static function getDashboardMetrics() {
        ...
        // Total Orders
        $stmt1 = $conn->prepare("SELECT COUNT(*) as count FROM `orders`");
        $stmt1->execute();
        $total_orders = $stmt1->get_result()->fetch_assoc()['count'];
        
        // Pending Orders
        $stmt2 = $conn->prepare("SELECT COUNT(*) as count FROM `orders` WHERE Status = 'Pending'");
        $stmt2->execute();
        $pending_orders = $stmt2->get_result()->fetch_assoc()['count'];
        
        // Total Revenue
        $stmt3 = $conn->prepare("SELECT COALESCE(SUM(TotalAmount), 0) as total FROM `orders` WHERE PaymentStatus = 'Paid'");
        $stmt3->execute();
        $total_revenue = $stmt3->get_result()->fetch_assoc()['total'];
        
        // Low Stock Items
        $stmt4 = $conn->prepare("SELECT COUNT(*) as count FROM inventory WHERE stock_quantity <= ReorderLevel");
        $stmt4->execute();
    ...
    public static function customerReport() {
        $user = authenticateUser();
        $start_date = isset($_GET['start_tDate']) ? $_GET['start_tDate'] : date('Y-m-01');
